...
/**
 * Validates Start to Finish GanttEntryRelationships.
 * 
 * 
 * 
 * @param <T>
 ...
 * @since 4.0
 */
public class StFRelationManagementController<T extends Comparable<T>, S extends UpdateableGanttEntry<T>>
		implements GanttRelationManagementController<T, S>
{
	...
	{
		// sucessor must be end after precessor has started
		if(sucessor.getRange().upper().compareTo(precessor.getRange().lower()) >= 0)
		{
			return true;
		}
